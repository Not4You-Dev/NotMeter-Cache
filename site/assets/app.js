(() => {
  ...
  const GITHUB_RANKING_RELEASE_ROOT =
    "https://github.com/Not4You-Dev/NotMeter-Cache/releases/download";
  let githubRankingPagesRoot = "";
  ...
  const ZH_TW_GAME_DATA_URL = "./assets/game-data.zh-TW.json?v=20260925-abyss";
  ...
  const DETAIL_TOTAL_TIMEOUT_MS = 20_000;
  const DETAIL_CACHE_READ_TIMEOUT_MS = 250;
  ...
  const DUNGEON_NAMES_EN = {
    ...
    "fire-temple-trial": "Trial: Fire Temple (Stage 16)",
  ...
  const DUNGEON_NAMES_ZH_TW = {
    ...
    "fire-temple-trial": "試煉：火之神殿（第16階段）",
  ...
  const FIRE_TEMPLE_DUNGEON = Object.freeze({ key: "fire-temple-trial", displayName: "시련: 불의 신전 (16단계)", bossNames: ["붉은불꽃 이그누스", "검은연기 무르트", "크로메데의 욕망"] });
  const PENDING_DUNGEON_KEYS = [...SNOWFIELD_STATISTICS_KEYS, FIRE_TEMPLE_DUNGEON.key];
  const FEATURED_DUNGEON_KEYS = [...SNOWFIELD_STATISTICS_KEYS, SNOWFIELD_DUNGEON.key, FIRE_TEMPLE_DUNGEON.key, "deus-research-hard", "noiran-legacy-4"];
  ...
  const COPY = {
    ko: {
      ...
      setupGuideSoulTitle: "부위별 영혼 각인", setupGuideSoulNote: "동일한 좌우 장신구는 한 부위로 합쳐 집계합니다. 인장은 유일(노란색) 이상 등급만 포함합니다.",
      setupGuideSealPending: "유일(노란색) 이상 인장 통계를 준비하고 있습니다.",
      ...
      classPerformanceKicker1000: "CP 1000K+ · PARTY GAP ≤100K · SHARE >10%",
      ...
      classPerformanceDescription1000: "이번 주 검증 기록 중 CP 1000K 이상·파티 CP 차이 100K 이하·기여도 10% 초과 표본만 같은 보스·10K CP 조건으로 맞춘 고CP 상대 지표입니다. 원시 DPS 순위가 아닙니다.",
      ...
      classPerformanceCpText1000: "이번 주 통계 검증을 통과한 보스 처치 중 서버·캐릭터·직업이 확인되고 CP가 1000K 이상인 기록만 사용합니다. 최대 CP 제한은 없습니다.",
      ...
      classPerformanceNoticeText1000: "CP 1000K 이상·파티 CP 차이 100K 이하·기여도 10% 초과 조건과 보스·중복 캐릭터·표본 부족 보정을 적용한 이번 주 고CP 실전 비교 지표입니다. A가 가장 안정적이고 C는 참고 가능한 최소 기준입니다. 파티 버프·보스별 역할·숙련도 차이까지 완전히 제거할 수는 없으므로 밸런스의 절대 판정이나 이론상 최대 DPS가 아니라, 실제 수집 기록에서 반복 관측된 경향으로 해석해 주세요.",
      ...
      rankerGuideDiamondChip: "전체 기간 특수 테두리 TOP 5",
      rankerGuideDiamondLabel: "움직이는 특수 테두리",
      rankerGuideDiamondTitle: "전체 기간 TOP 5",
      rankerGuideDiamondBody: "전체 기간 1~5위일 때만 다이아몬드가 움직입니다. 이번 주 TOP 5만으로는 이 효과가 생기지 않습니다.",
      rankerPreviewHeading: "직업 아이콘 테두리, 이렇게 달라집니다",
      rankerPreviewIntro: "아이콘 숫자는 전체 기간과 이번 주 중 더 높은 순위(더 작은 숫자)입니다. 같은 검성 아이콘으로 비교했으며, 숫자 색은 미터기 설정에 따라 달라질 수 있습니다.",
      rankerPreviewWeeklyTitle: "이번 주 1~5위",
      rankerPreviewWeeklyBody: "금색 모서리와 오른쪽 위 보석. 전체 기간 TOP 5가 아닐 때의 모습입니다.",
      rankerPreviewWeeklyAlt: "금색 모서리에 숫자 5가 표시된 검성 아이콘",
      rankerPreviewAlltimeTitle: "전체 기간 1~5위",
      rankerPreviewAlltimeBody: "금색 다이아몬드와 하단 광선. 광점이 테두리를 따라 움직이며, 일반 모서리보다 우선 표시됩니다.",
      rankerPreviewAlltimeAlt: "금색 다이아몬드에 숫자 5가 표시된 검성 아이콘",
      rankerPreviewTenTitle: "6~10위",
      rankerPreviewTenBody: "하늘색 모서리와 오른쪽 위 보석. 전체 기간·이번 주에 공통으로 적용됩니다.",
      rankerPreviewTenAlt: "하늘색 모서리에 숫자 10이 표시된 검성 아이콘",
      rankerPreviewTwentyTitle: "11~20위",
      rankerPreviewTwentyBody: "보라색 모서리와 오른쪽 위 보석. 전체 기간·이번 주에 공통으로 적용됩니다.",
      rankerPreviewTwentyAlt: "보라색 모서리에 숫자 20이 표시된 검성 아이콘",
      rankerPreviewCombinedTitle: "전체 기간 5위 + 이번 주 1위라면?",
      rankerPreviewCombinedBody: "숫자는 더 높은 순위인 1, 테두리는 전체 기간 TOP 5 다이아몬드입니다. 전체 기간이 6위로 내려가고 이번 주 1위만 남으면 숫자 1은 유지되고 금색 모서리로 바뀝니다.",
      rankerPreviewCombinedAlt: "전체 기간 TOP 5 다이아몬드에 더 높은 순위인 숫자 1이 표시된 검성 아이콘",
      rankerPreviewPlay: "움직임 보기",
      rankerPreviewPause: "멈추기",
      rankerPreviewMotionAria: "{title} · {action}",
      rankerPreviewMotionNote: "움직임 줄이기를 설정했다면 정지 이미지로 시작합니다. ‘움직임 보기’를 눌렀을 때만 재생하며, 안내를 닫으면 멈춥니다.",
      rankerGuideOtherTitle: "칭찬 랭킹·쿠폰은 별도 권한입니다",
      rankerGuidePraiseBody: "칭찬 랭킹 TOP 20을 유지하는 동안 모든 닉네임 효과를 사용할 수 있습니다. CP·직업 조건은 없으며, 랭커 마크나 다이아몬드 테두리를 주는 혜택은 아닙니다.",
      rankerGuideCouponBody: "칭찬 TOP 20에서 벗어나면 칭찬 혜택만 종료됩니다. 유효한 쿠폰과 별도로 충족한 랭킹 권한은 유지되며, 쿠폰 효과에는 CP 제한이 없습니다.",
      rankerGuideFrozenExcluded: "무스펠의 성배와 시련: 바크론의 공중섬은 랭커 마크·랭킹 닉네임 효과 대상에서 제외됩니다. 비탄의 설원은 보통·어려움의 트리톤과 델트라스 2페이즈만 해당합니다.",
      ...
      rankerDungeonGuideMarkerBody: "CP 800K 이상에서 대상 보스·동일 직업·현재 CP 구간의 두 기간 중 하나가 TOP 20이면 표시됩니다. CP는 25K 단위이며 2M 이상은 통합 구간입니다. 미터기의 랭커 표시 설정도 켜져 있어야 합니다.",
      ...
      rankerDungeonGuideRetentionBody: "현재 CP 구간에서 TOP 3 자격을 잃고 다른 유효한 권한도 없으면 효과가 숨겨집니다. 선택한 효과는 지워지지 않으며, 자격을 다시 확인하면 복원됩니다. 유효한 쿠폰·칭찬 혜택은 별도로 유지됩니다.",
      ...
      rankerDungeonFireTempleTitle: "시련: 불의 신전 (16단계)",
      rankerDungeonFireTempleBosses: "붉은불꽃 이그누스 · 검은연기 무르트 · 크로메데의 욕망",
      rankerDungeonFireTempleRule: "단계가 확인된 16단계 처치기록만 집계합니다. 세 보스 각각 상위 %와 기존 구간 랭킹·닉네임 효과 규칙을 적용합니다.",
      ...
      rankerDungeonGuideDummy: "훈련용 허수아비(1분)는 홈페이지 랭킹만 제공하며, 딜미터기 전투 종료 구간 순위·상위% 배지와 실시간 랭커 마크 대상에서는 제외됩니다. 단, CP 800K 이상 구간 TOP 3의 닉네임 효과 자격은 별도로 제공합니다.",
      ...
      setupUsageCount: "{rate} · {count}/{total}명 사용",
      setupGuideCategoryAria: "세팅 항목 선택",
      setupGuideStonesTab: "마석 · 영석",
      setupGuideSoulTab: "영혼 각인",
      setupGuideSkillsTab: "스킬 · 특성",
      ...
      chuseokCouponTitle: "아이온2 추석 쿠폰 보상",
      chuseokCouponPeriod: "9월 23일 수요일 정기점검 이후 사용 가능합니다. (~10/14 수요일까지)",
      couponCopy: "쿠폰 복사",
      couponRewards: "보상 보기",
      couponRewardsOriginal: "원본 보기",
      couponRewardsClose: "닫기",
      couponCopied: "복사됨",
      couponCopyFailed: "직접 복사",
      couponCopyHelp: "자동 복사가 지원되지 않습니다. 쿠폰 번호를 선택해 직접 복사해 주세요.",
      pairedObserved: "관측 {value}",
      pairedEstimate: "관측 변화율 {value}",
      pairedDirectionUnclear: "추정 범위가 0%를 포함하여 상승·하락 판단은 보류합니다. 주간 마감 직전에도 나타날 수 있으며, 집계 대기나 표본 부족을 뜻하지 않습니다.",
      ...
      weeklyGuideMeaning: "조건별 최근 40회 이내 기록의 중앙값으로 캐릭터별 변화율을 구한 뒤, 각 캐릭터에 같은 비중을 주어 중앙값을 표시합니다. 비교 캐릭터 20명 이상이면 계산된 변화율을 표시합니다. 추정 범위가 0%를 포함하면 회색 ‘관측’으로 표시하고 상승·하락 판단만 보류합니다. 주간 마감 직전에도 나타날 수 있으며 집계 대기를 뜻하지 않습니다. 20명 미만은 비교 인원을 표시합니다. 범위는 파티 간 상관관계까지 보장하지 않습니다.",
    ...
    en: {
      ...
      setupGuideSoulTitle: "Soul engravings by slot", setupGuideSoulNote: "Matching accessories are combined into one slot. Seals include only Unique (yellow) or higher grades.",
      setupGuideSealPending: "Statistics for Unique (yellow) or higher seals are being prepared.",
      ...
      discord: "Discord",
      communityDiscord: "AION2 Discord",
      download: "Download",
      ...
      classPerformanceDescription1000: "A high-CP relative index using only this week's verified samples with 1000K+ CP, a party CP spread of 100K or less, and over 10% damage contribution, matched by boss and 10K CP bracket. It is not a raw-DPS leaderboard.",
      ...
      classPerformanceCpText1000: "Only accepted boss kills with a known server, character, and class and record-time CP of at least 1000K are used. There is no maximum CP limit.",
      ...
      classPerformanceScopeStatus1000: "Showing only samples at CP 1000K+",
      classPerformanceScopeSummary800: "CP 800K+",
      classPerformanceScopeSummary900: "CP 900K+",
      classPerformanceScopeSummary1000: "CP 1000K+",
      ...
      classPerformanceNoticeText1000: "This high-CP weekly comparison applies 1000K+ CP, a party CP spread of 100K or less, over 10% damage contribution, boss matching, character deduplication, and small-sample controls. A is the most stable grade, while C is the minimum useful reference. Party buffs, encounter roles, and player skill cannot be removed completely, so this is a repeatedly observed trend rather than an absolute balance verdict or theoretical maximum DPS.",
      ...
      rankerGuideDiamondChip: "All-time special border: Top 5",
      rankerGuideDiamondLabel: "ANIMATED BORDER",
      rankerGuideDiamondTitle: "All-time Top 5",
      rankerGuideDiamondBody: "The animated diamond requires an all-time rank of 1–5. A weekly Top 5 rank alone does not grant it.",
      rankerPreviewHeading: "Compare the class-icon borders",
      rankerPreviewIntro: "The icon shows the better rank (smaller number) across all-time and this week. These examples use the same Gladiator icon; number colors may vary with your meter settings.",
      rankerPreviewWeeklyTitle: "This week: ranks 1–5",
      rankerPreviewWeeklyBody: "Gold corners and a gem at the top right, when you are not in the all-time Top 5.",
      rankerPreviewWeeklyAlt: "Gladiator icon with gold corners and the number 5",
      rankerPreviewAlltimeTitle: "All-time: ranks 1–5",
      rankerPreviewAlltimeBody: "A gold diamond, a beam below, and a light moving around the border. This takes priority over the regular corners.",
      rankerPreviewAlltimeAlt: "Gladiator icon with a gold diamond and the number 5",
      rankerPreviewTenTitle: "Ranks 6–10",
      rankerPreviewTenBody: "Sky-blue corners and a gem at the top right. The same border applies to either period.",
      rankerPreviewTenAlt: "Gladiator icon with sky-blue corners and the number 10",
      rankerPreviewTwentyTitle: "Ranks 11–20",
      rankerPreviewTwentyBody: "Purple corners and a gem at the top right. The same border applies to either period.",
      rankerPreviewTwentyAlt: "Gladiator icon with purple corners and the number 20",
      rankerPreviewCombinedTitle: "All-time #5 + this week's #1?",
      rankerPreviewCombinedBody: "The icon shows the better rank, 1, with the all-time Top 5 diamond. If the all-time rank drops to 6 while the weekly rank stays at 1, the number remains 1 but the border changes to gold corners.",
      rankerPreviewCombinedAlt: "Gladiator icon with the all-time Top 5 diamond and the better rank, 1",
      rankerPreviewPlay: "Play animation",
      rankerPreviewPause: "Pause animation",
      rankerPreviewMotionAria: "{title} · {action}",
      rankerPreviewMotionNote: "With reduced motion enabled, previews start still. Play an animation only when you want to; closing this guide stops it.",
      rankerGuideOtherTitle: "Praise ranking and coupons are separate benefits",
      rankerGuidePraiseBody: "While you remain in the praise-ranking Top 20, you can use every nickname effect, with no CP or class requirement. This does not grant a combat rank marker or diamond border.",
      rankerGuideCouponBody: "Leaving the praise Top 20 removes only that benefit. Valid coupons and independently earned combat-ranking access remain valid. Coupon effects have no CP restriction.",
      rankerGuideFrozenExcluded: "Musphel's Grail and Trial: Bakron's Sky Island are excluded from rank markers and ranking-based nickname effects. Snowfield eligibility covers only Triton and Deltras phase 2 on Normal and Hard.",
      ...
      rankerDungeonGuideMarkerBody: "At 800K CP or above, reach Top 20 in either period for an eligible boss, the same class, and your current CP bracket. Brackets are 25K wide; 2M+ is merged. Enable rank markers in the meter settings too.",
      ...
      rankerDungeonGuideRetentionBody: "The effect is hidden if you lose Top 3 eligibility in your current CP bracket and have no other valid access. Your selection is kept and returns once eligibility is verified again. Active coupon and praise benefits remain independent.",
      ...
      rankerDungeonFireTempleTitle: "Trial: Fire Temple (Stage 16)",
      rankerDungeonFireTempleBosses: "Red Spark Ignus · Black Smoke Murute · Kromede's Desire",
      rankerDungeonFireTempleRule: "Only confirmed Stage 16 kills are counted. Each of the three bosses uses the existing Top %, bracket ranking, and nickname-effect rules.",
      ...
      rankerDungeonGuideDummy: "Training Dummy (1 min) provides website rankings only. It does not show the meter's post-combat bracket-rank or Top % badge, and it does not award a live rank marker. Its bracket Top 3 at 800K CP or above separately qualifies for nickname effects.",
      ...
      setupUsageCount: "{rate} · {count}/{total} characters",
      setupGuideCategoryAria: "Choose a setup category",
      setupGuideStonesTab: "Stones",
      setupGuideSoulTab: "Soul engravings",
      setupGuideSkillsTab: "Skills & traits",
      ...
      chuseokCouponTitle: "AION2 Chuseok reward coupon",
      chuseokCouponPeriod: "Available after scheduled maintenance on Wed, Sep 23, through Wed, Oct 14 (KST).",
      couponCopy: "Copy coupon",
      couponRewards: "View rewards",
      couponRewardsOriginal: "Full image",
      couponRewardsClose: "Close",
      couponCopied: "Copied",
      couponCopyFailed: "Copy manually",
      couponCopyHelp: "Automatic copying is unavailable. Select and copy the coupon code manually.",
      pairedObserved: "Observed {value}",
      pairedEstimate: "Observed change {value}",
      pairedDirectionUnclear: "The range includes zero, so the direction is inconclusive. This can persist until the weekly reset; it does not mean pending data or too few matches.",
      ...
      weeklyUp: "Higher",
      weeklyDown: "Lower",
      weeklyFlat: "Observed · direction inconclusive",
      ...
      weeklyGuideMeaning: "Compare medians of up to 40 recent runs per matched condition, then take the median change with one vote per character. With at least 20 matches, the calculated change is shown. If the range includes zero, a gray Observed value remains visible while the direction is inconclusive, even just before the weekly reset. This is not pending data. Fewer than 20 matches is insufficient and shows the matched count. The range does not account for party correlation.",
      ...
      detailHealingShare: "Healing share",
  ...
  Object.assign(COPY.ko, {
    musphelStatisticsEnded: "더 이상 무스펠의 성배(어려움) 통계를 지원하지 않습니다. 신규 전투 기록은 업로드되지 않습니다.",
    bakronStatisticsEnded: "2026년 9월 23일 오전 5시(한국시간)부터 시련: 바크론의 공중섬 신규 전투 기록은 통계에 업로드되지 않습니다.",
    partyFinderTitle: "성역·원정 파티원 찾기",
    partyFinderDescription: "고정 파티 모집부터 함께할 파티원 찾기까지",
    partyFinderDiscord: "통합 디스코드에서 파티 찾기",
    dungeonDifficulty: "난이도",
    dungeonDifficultyAria: "비탄의 설원 난이도 선택",
    difficultyNormal: "보통",
    difficultyHard: "어려움",
    dungeonStatisticsPending: "선택한 던전의 통계가 아직 게시되지 않았습니다. 게시되면 이곳에 표시됩니다.",
  ...
  Object.assign(COPY.en, {
    musphelStatisticsEnded: "Statistics for Musphel's Grail (Hard) are no longer supported. New combat records are no longer uploaded.",
    bakronStatisticsEnded: "From September 23, 2026 at 05:00 KST, new combat records for Trial: Bakron's Sky Island will no longer be uploaded to statistics.",
    partyFinderTitle: "Sanctuary & Expedition parties",
    partyFinderDescription: "Recruit a fixed group or find players to join your party",
    partyFinderDiscord: "Find a party on AION2 Discord",
    dungeonDifficulty: "Difficulty",
    dungeonDifficultyAria: "Select Snowfield of Sorrow difficulty",
    difficultyNormal: "Normal",
    difficultyHard: "Hard",
    dungeonStatisticsPending: "Statistics for this dungeon have not been published yet. They will appear here once available.",
  ...
  Object.assign(COPY["zh-TW"], {
    musphelStatisticsEnded: "已停止支援穆斯費爾聖杯（困難）的統計，不再上傳新的戰鬥紀錄。",
    bakronStatisticsEnded: "自2026年9月23日上午5時（韓國時間）起，試煉：巴克隆空中島的新戰鬥紀錄將不再上傳至統計。",
    partyFinderTitle: "聖域・遠征隊友招募",
    partyFinderDescription: "招募固定團，尋找一起冒險的隊友",
    partyFinderDiscord: "前往綜合 Discord 尋找隊友",
    dungeonDifficulty: "難度",
    dungeonDifficultyAria: "選擇悲嘆雪原難度",
    difficultyNormal: "普通",
    difficultyHard: "困難",
    dungeonStatisticsPending: "此副本的統計尚未公布，公布後將顯示於此。",
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    bindEvents();
    applyLocale();
    window.NotMeterCouponNotice?.init(t);
    window.NotMeterRankerGuide?.init(t);
    ...
    if (pageView === "praise-ranking") {
      openPraiseRankingView();
    } else if (pageView === "character") {
      openCharacterView();
  ...
  function bindElements() {
    for (const id of [
      ...
      "contribution-dungeon-title", "contribution-rows", "setup-guide-categories", "setup-guide-snapshot-range",
      ...
      "dungeon-filter", "dungeon-filter-buttons",
      ...
      "dungeon-support-notice", "dungeon-support-notice-message", "party-finder-guide",
  ...
  function bindEvents() {
    ...
    elements["contribution-tabs"].addEventListener("click", event => {
      ...
      elements["contribution-tabs"].querySelector('[aria-pressed="true"]')?.focus({ preventScroll: true });
    });
    elements["setup-guide-categories"].addEventListener("click", event => {
      const button = event.target.closest("[data-setup-category]");
      if (button) selectSetupGuideCategory(button.dataset.setupCategory);
    });
    ...
    document.querySelectorAll("[data-performance-minimum-cp]").forEach(button => {
      button.addEventListener("click", () => {
        ...
        if (![800_000, 900_000, 1_000_000].includes(minimumCombatPower)) {
          return;
        }
  ...
  function handlePageHistory(event) {
      ...
      if (view === "praise-ranking") return openPraiseRankingView();
      closePraiseRankingView();
  ...
  function localizeGameName(value, type = "", ...codes) {
    ...
    if (state.locale === "en") {
      if (original === "붉은불꽃 이그누스") return "Red Spark Ignus";
      if (original === "검은연기 무르트") return "Black Smoke Murute";
      if (original === "크로메데의 욕망") return "Kromede's Desire";
  ...
  function githubCacheCandidates(path, rawUrl) {
    if (githubRankingPagesRoot) {
      encodeReleaseAssetPath(path);
      return [`${githubRankingPagesRoot}/${path}`];
    }
  ...
  function rankingCacheCandidates(path, rawUrl) {
    if (githubRankingPagesRoot) return githubCacheCandidates(path, rawUrl);
    return [
    ...
    const load = (async () => {
      ...
      for (const baseUrl of GITHUB_RANKING_MANIFEST_URLS) {
        ...
        try {
          ...
          validateGitHubRankingPages(candidate);
          if (githubRankingManifestPublishedAt > 0 &&
              Date.parse(String(candidate.publishedAt || "")) < githubRankingManifestPublishedAt) {
            throw new Error("older GitHub cache manifest");
          }
          manifest = candidate;
          break;
        } catch (error) {
      ...
      githubRankingPagesRoot = validateGitHubRankingPages(manifest);
  ...
  function validateGitHubRankingPages(manifest) {
    if (!manifest.storage) return "";
    const generation = String(manifest.release?.generation || "");
    if (manifest.storage !== "github-pages-artifact-v1" || !/^[0-9a-f]{12,64}$/.test(generation) ||
        ![`https://not4you-dev.github.io/NotMeter-Cache/g/${generation}`,
  ...
  async function loadCache(force = false, preserveView = false) {
    ...
    try {
      ...
      if (isMatchingClassOverallCache(classOverallCache, cache.generatedAt)) {
        ...
        cache.classPerformance1000K = classOverallCache.classPerformance1000K;
      ...
      const nextDungeon = cache.dungeons.some(item => item.key === previousDungeon) ||
          PENDING_DUNGEON_KEYS.includes(previousDungeon)
        ? previousDungeon
      ...
      const initialViews = isPendingDungeon(nextDungeon, cache) ? [] : initialDungeonViews.length > 0
        ? initialDungeonViews
      ...
      if (state.cpFilterMode === "custom" && (generationChanged || force) &&
          !isPendingDungeon(nextDungeon, cache)) {
        try {
  ...
  function initialPageView() {
    ...
    return view === "class-top10" || view === "field-boss" || view === "artifact" || view === "optimization" ||
      ...
      view === "character" || view === "praise-ranking" ? view : "ranking";
  }

  function openPraiseRankingView() {
    closeFieldBossView(); closeArtifactView(); closeOptimizationView(); closeClassTop10View();
    closeClassPerformanceView(); closeContributionView(); closeBossResistanceView(); closeStatEfficiencyView();
    state.surfaceMode = "praiseRanking";
    document.body.classList.add("praise-ranking-view");
    document.getElementById("praise-ranking-surface").hidden = false;
    document.getElementById("praise-ranking-button").setAttribute("aria-current", "page");
    window.NotMeterPraiseRanking?.activate({ serverLabel, jobIcon: value => {
      const canonical = Object.keys(JOB_NAMES_EN).find(key =>
        JOB_NAMES_EN[key] === (value === "Elementalist" ? "Spiritmaster" : value)) || value;
      return JOB_ORDER.includes(canonical)
        ? { src: `./assets/jobs/${encodeURIComponent(canonical)}.png`, label: jobName(canonical) }
        : null;
    } });
    updatePageIdentity();
  }

  function closePraiseRankingView() {
    if (state.surfaceMode !== "praiseRanking") return;
    state.surfaceMode = "ranking";
    document.body.classList.remove("praise-ranking-view");
    document.getElementById("praise-ranking-surface").hidden = true;
    document.getElementById("praise-ranking-button").setAttribute("aria-current", "false");
  }
    ...
    const servers = cache.servers.map(server => ({
      ...
      regions: (server?.regions || []).map(region => ({
        ...
        entries: (region?.entries || []).map(entry => {
          if (FIELD_BOSS_FIXED_SCHEDULES[normalizeFieldBossCode(entry?.bossCode)]) return entry;
  ...
  function resolveFieldBossTargetAt(bossCode, candidateTargetAt, now = Date.now()) {
    ...
    const schedule = FIELD_BOSS_FIXED_SCHEDULES[normalizeFieldBossCode(bossCode)];
    ...
    const anchor = koreaDayStart + koreaStartHour * hourMilliseconds
      + (Number(schedule.koreaStartMinute) || 0) * 60_000;
    if (Array.isArray(schedule.weekdays)) {
      for (let daysAhead = 0; daysAhead <= 7; daysAhead++) {
        const target = anchor + daysAhead * dayMilliseconds;
        if (target > koreaTime && schedule.weekdays.includes(new Date(target).getUTCDay())) {
          return target - koreaOffsetMilliseconds;
        }
      }
      return candidate;
    }
    if (!Number.isInteger(intervalHours) || intervalHours <= 0) return candidate;
      ...
      servers: cache.servers.map(server => ({
        ...
        regions: Array.isArray(server?.regions)
          ? server.regions.map(region => {
              ...
              const sourceEntries = Array.isArray(region?.entries) ? region.entries : [];
              const sourceCodes = new Set(sourceEntries.map(entry => Number(entry?.bossCode)));
              const separateExecutors = sourceCodes.has(2600097) && sourceCodes.has(2600098);
              for (const entry of sourceEntries) {
                const originalBossCode = Number(entry?.bossCode);
                if (originalBossCode === 2600068 && !separateExecutors) continue;
  ...
  function currentFieldBossServer(now = Date.now()) {
    const server = state.fieldBossData?.servers?.find(
      server => Number(server.serverId) === state.fieldBossServerId);
    const regions = new Map((server?.regions || []).map(region => [Number(region.region), region]));
    FIELD_BOSS_REGIONS.forEach((catalog, regionIndex) => {
      const fixedBosses = catalog.bosses.filter(([code]) => FIELD_BOSS_FIXED_SCHEDULES[code]);
      if (!fixedBosses.length) return;
      const region = regions.get(regionIndex) || { region: regionIndex, observedAt: 0, entries: [] };
      const entries = new Map(region.entries.map(entry => [Number(entry.bossCode), entry]));
      for (const [bossCode] of fixedBosses) {
        entries.set(bossCode, {
          ...entries.get(bossCode), bossCode,
          targetAt: resolveFieldBossTargetAt(bossCode, 0, now),
        });
      }
      regions.set(regionIndex, { ...region, entries: [...entries.values()] });
    });
    return { ...server, serverId: state.fieldBossServerId, regions: [...regions.values()] };
  }
  ...
  function isSetupGuideCache(cache) {
    return cache?.schema === EXPECTED_CONTRIBUTION_SCHEMA &&
      [1, 2, 3].includes(Number(cache.version)) && Array.isArray(cache.jobs);
  }
  ...
  function renderContributionStats() {
    ...
    elements["setup-guide-snapshot-range"].textContent = t("setupSnapshotRange", { range: snapshotRange });
    ...
    stones.className = "setup-guide-stones";
    stones.append(renderSetupGuideStats(
    ...
    const panels = [
      ["stones", stones],
      ["soul", renderSetupGuideSoul(selected.soulEngravings)],
      ["arcana", renderSetupGuideArcana(selected.arcana)],
      ["skills", renderSetupGuideSkills(selected.skills)],
    ];
    for (const [key, panel] of panels) {
      panel.id = `setup-guide-panel-${key}`;
      panel.dataset.setupPanel = key;
      rows.append(panel);
    }
    elements["contribution-rows"].replaceChildren(rows);
    selectSetupGuideCategory(state.setupGuideCategory);
    showContributionState("content");
  }

  function selectSetupGuideCategory(key) {
    const buttons = elements["setup-guide-categories"].querySelectorAll("[data-setup-category]");
    const selected = Array.from(buttons).some(button => button.dataset.setupCategory === key) ? key : "stones";
    state.setupGuideCategory = selected;
    for (const button of buttons) {
      button.setAttribute("aria-pressed", String(button.dataset.setupCategory === selected));
    }
    for (const panel of elements["contribution-rows"].children) {
  ...
  function setupGuideSoulSlots(slots, version) {
    const source = Array.isArray(slots) ? slots : [];
    if (Number(version) >= 3) return source;
    const isSeal = slot => [25, 26].includes(Number(slot?.slotPos));
    const first = source.findIndex(isSeal);
    // Older caches contain fixed blue-grade options and cannot be filtered by grade.
    return source.flatMap((slot, index) => index === first
      ? [{ slotPos: 25, slotName: "인장", pendingGradeFilter: true }]
      : isSeal(slot) ? [] : [slot]);
  }
  ...
  function renderSetupGuideSoul(slots) {
    const { section, body } = createSetupGuideSection(
      ...
      "setupGuideSoulNote");
    const source = setupGuideSoulSlots(slots, state.contributionData?.version);
    ...
    for (const [groupTitleKey, groupedSlots] of slotGroups) {
      ...
      for (const slot of visibleSlots) {
        ...
        group.dataset.setupSlot = String(slot.slotPos);
        const title = document.createElement("h4");
        title.textContent = setupGuideEquipmentSlotName(slot);
        group.append(title);
        if (slot.pendingGradeFilter) {
          const pending = document.createElement("p");
          pending.className = "setup-guide-inline-empty";
          pending.textContent = t("setupGuideSealPending");
    ...
    organizeSetupGuideSlots(body, "soul", "setupGuideSoulTitle");
    if (!body.childElementCount) appendSetupGuideEmpty(body);
    return section;
  }
  ...
  function renderSetupGuideArcana(arcana) {
    ...
    organizeSetupGuideSlots(body, "arcana", "setupGuideArcanaSlots");
    if (!body.childElementCount) appendSetupGuideEmpty(body);
    return section;
  }

  function organizeSetupGuideSlots(body, kind, labelKey) {
    const grids = Array.from(body.querySelectorAll(".setup-guide-slot-grid"));
    if (!grids.length) return;
    const browser = document.createElement("div");
    browser.className = "setup-guide-slot-browser";
    const nav = document.createElement("nav");
    nav.className = "setup-guide-slot-nav";
    nav.setAttribute("aria-label", t(labelKey));
    const detail = document.createElement("div");
    detail.className = "setup-guide-slot-detail";
    const slots = [];
    const buttons = [];
    for (const grid of grids) {
      ...
      choices.className = "setup-guide-slot-choices";
      if (heading) nav.append(heading);
      for (const slot of Array.from(grid.children)) {
        const button = document.createElement("button");
        button.type = "button";
        button.textContent = slot.querySelector("h4").textContent;
        button.dataset.setupSlot = slot.dataset.setupSlot;
        slot.id = `setup-guide-${kind}-slot-${slot.dataset.setupSlot}`;
    ...
    const select = key => {
      const selected = slots.some(slot => slot.dataset.setupSlot === key) ? key : slots[0]?.dataset.setupSlot;
      state.setupGuideSlots[kind] = selected;
      slots.forEach(slot => { slot.hidden = slot.dataset.setupSlot !== selected; });
      buttons.forEach(button => button.setAttribute("aria-pressed", String(button.dataset.setupSlot === selected)));
    };
    nav.addEventListener("click", event => {
      const button = event.target.closest("button[data-setup-slot]");
      if (button) select(button.dataset.setupSlot);
    });
    select(state.setupGuideSlots[kind]);
    browser.append(nav, detail);
    body.prepend(browser);
  }
  ...
  function dungeonFilterItems() {
    ...
    const snowfield = dungeons.some(dungeon => SNOWFIELD_STATISTICS_KEYS.includes(dungeon.key))
      ? SNOWFIELD_DUNGEON
      : dungeons.find(dungeon => dungeon.key === SNOWFIELD_DUNGEON.key) || SNOWFIELD_DUNGEON;
    const fireTemple = dungeons.find(dungeon => dungeon.key === FIRE_TEMPLE_DUNGEON.key) || FIRE_TEMPLE_DUNGEON;
    return [snowfield, fireTemple, ...dungeons.filter(dungeon =>
      dungeon.key !== SNOWFIELD_DUNGEON.key && dungeon.key !== FIRE_TEMPLE_DUNGEON.key &&
      !SNOWFIELD_STATISTICS_KEYS.includes(dungeon.key))];
  }
  ...
  function renderDungeonFilterButtons() {
    ...
    for (const dungeon of dungeons) {
      ...
      button.append(label);
      if (dungeon.key === SNOWFIELD_DUNGEON.key || dungeon.key === FIRE_TEMPLE_DUNGEON.key || SNOWFIELD_STATISTICS_KEYS.includes(dungeon.key)) {
  ...
  function renderBossCombatStats() {
    const notice = elements["dungeon-support-notice"];
    if (notice) {
      const noticeKey = state.dungeonKey === "musphel-hard" ? "musphelStatisticsEnded"
        : state.dungeonKey === "bakron-trial" ? "bakronStatisticsEnded" : "";
      notice.hidden = state.surfaceMode !== "ranking" || !noticeKey;
      elements["dungeon-support-notice-message"].textContent = notice.hidden ? "" : t(noticeKey);
    }
    ...
    const partyFinder = elements["party-finder-guide"];
    if (partyFinder) partyFinder.hidden = panel.hidden;
  ...
  function syncClassPerformanceScope() {
    const scope = state.performanceMinimumCombatPower === 1_000_000 ? "1000"
      : state.performanceMinimumCombatPower === 900_000 ? "900" : "800";
    document.querySelectorAll("[data-performance-minimum-cp]").forEach(button => {
    ...
    elements["class-performance-kicker"].textContent = t(
      `classPerformanceKicker${scope}`);
    elements["class-performance-description"].textContent = t(
      `classPerformanceDescription${scope === "800" ? "" : scope}`);
    elements["class-performance-cp-text"].textContent = t(
      `classPerformanceCpText${scope === "800" ? "" : scope}`);
    elements["class-performance-notice-text"].textContent = t(
      `classPerformanceNoticeText${scope === "800" ? "" : scope}`);
    elements["class-performance-scope-status"].textContent = t(
      `classPerformanceScopeStatus${scope}`);
    return {
      snapshot: state.data?.[scope === "800" ? "classPerformance" : `classPerformance${scope}K`],
      summaryKey: `classPerformanceScopeSummary${scope}`,
    };
  }
  ...
  function renderClassPerformance() {
    ...
    const available = Boolean(snapshot && Array.isArray(snapshot.rows));
  ...
  function openLegacyCombatDetail(player, detail) {
    ...
    state.selectedDetail = {
      ...
      record: {
        bossName,
        comparisonDurationKnown: Number.isFinite(Number(player.durationSeconds)) && Number(player.durationSeconds) > 0,
  ...
  function openUnavailableCombatDetail(player, reason, loading = false) {
    const dungeon = currentDungeon();
    const bossIndex = Number(player.B ?? player.bossIndex ?? state.bossIndex);
    const bossName = localizeGameName(bossIndex > 0
      ? dungeon?.bossNames?.[bossIndex - 1]
  ...
  async function openRemoteCombatDetail(player, lookupKey, row, detailContext) {
    if (!lookupKey || (state.selectedDetail?.loading && state.selectedDetail.player === player)) {
      return;
    }
    const selection = openUnavailableCombatDetail(player, "", true);
    row.classList.add("detail-loading");
    row.setAttribute("aria-busy", "true");
    const detailLink = row.querySelector(".detail-link");
    const previousLabel = `${t("viewDetails")} ›`;
    if (detailLink) {
      detailLink.textContent = t("detailLoading");
    }
    try {
      lookupKey = await lookupKey;
      if (state.selectedDetail !== selection) return;
      ...
      selection.unavailableReason = reason;
      renderCombatDetail();
    } finally {
  ...
  async function loadRankingCombatDetailCore(
    ...
    const cacheLoad = "caches" in window
      ? Promise.resolve().then(() => caches.open(DETAIL_CACHE_NAME)).catch(() => null)
      : Promise.resolve(null);
    const cachedDocument = await waitForDetailCache((async () => {
      const cache = await cacheLoad;
      const cachedResponse = await cache?.match(request);
      return cachedResponse
        ? parseRankingCombatDetail(cachedResponse, lookupKey, expectedDungeonKey)
        : null;
    })());
    if (cachedDocument) {
    ...
    const { detailDocument, cacheCopy } = await downloadRankingCombatDetail(
      ...
      lookupKey,
      expectedDungeonKey);
    void cacheLoad.then(cache => cache?.put(request, cacheCopy)).catch(() => {});
    return detailDocument;
  }

  async function waitForDetailCache(operation) {
    let timer;
    try {
      return await Promise.race([
        operation.catch(() => null),
        new Promise(resolve => { timer = window.setTimeout(() => resolve(null), DETAIL_CACHE_READ_TIMEOUT_MS); }),
      ]);
    } finally {
  ...
  async function downloadRankingCombatDetail(
    ...
    expectedDungeonKey) {
    const controller = new AbortController();
    const timeout = window.setTimeout(() => controller.abort(), DETAIL_TOTAL_TIMEOUT_MS);
    try {
      return await withDetailAbort(
        requestRankingCombatDetail(requestPath, lookupKey, expectedDungeonKey, controller.signal),
        controller.signal);
    } finally {
  ...
  async function requestRankingCombatDetail(requestPath, lookupKey, expectedDungeonKey, signal) {
    const failures = [];
    const statuses = [];
    const endpoints = await withDetailAbort(resolveRankingDetailEndpoints(), signal);
    let retryDelayMs = 0;
    for (let attempt = 0; attempt < 2; attempt += 1) {
      for (const endpoint of endpoints) {
        signal.throwIfAborted();
        const delayMs = retryDelayMs || (attempt > 0 ? DETAIL_RETRY_DELAY_MS : 0);
        if (delayMs > 0) {
          let delayTimer;
          try {
            await withDetailAbort(new Promise(resolve => { delayTimer = window.setTimeout(resolve, delayMs); }), signal);
          } finally {
        ...
        try {
          ...
          if ((Number.isInteger(error?.status) && ![408, 429, 500, 502, 503, 504].includes(error.status)) ||
              error?.retryAfterSeconds > DETAIL_TOTAL_TIMEOUT_MS / 1000) throw error;
          retryDelayMs = error?.status === 429
            ? Math.max(DETAIL_RETRY_DELAY_MS, error.retryAfterSeconds * 1000)
            : 0;
  ...
  function withDetailAbort(operation, signal) {
  ...
  function closeCombatDetail() {
    globalThis.NotMeterDummyComparison?.close();
    if (!state.selectedDetail && elements["combat-detail-modal"]?.hidden) {
      return;
    }
    state.selectedDetail = null;
    elements["combat-detail-modal"]?.removeAttribute("aria-busy");
    resetCombatDetailScrollPosition();
  ...
  function renderCombatDetail() {
    ...
    const loading = state.selectedDetail.loading === true;
    const unavailableReason = loading ? t("detailLoading") : String(state.selectedDetail.unavailableReason || "");
    elements["combat-detail-modal"].setAttribute("aria-busy", String(loading));
    ...
    const skills = Array.isArray(detail.skills)
      ? detail.skills.map(globalThis.NotMeterCombatDetailBuffs.normalizeSkill)
          .filter(skill =>
      ...
      : [];
    globalThis.NotMeterDummyComparison?.sync({
      record, detail, dungeonKey: state.dungeonKey, locale: state.locale, unavailable: Boolean(unavailableReason),
      icon: applyDetailSkillIcon,
      specialization: buildSpecializationChip,
      showSkill: skill => globalThis.NotMeterCombatDetailBuffs.shouldDisplaySkill(skill),
      skillName: skill => globalThis.NotMeterCombatDetailBuffs.verifiedSkillName(skill, state.locale) || localizeGameName(
        globalThis.NotMeterCombatDetailBuffs?.skillDisplayName?.(skill, state.locale) || String(skill.skillName || "—"),
    ...
    if (unavailableReason) {
      ...
      const title = document.createElement("strong");
      title.textContent = t(loading ? "combatDetails" : "detailUnavailableTitle");
  ...
  function buildDetailMetricColumns(skills) {
    ...
    add("healing", "healing", "healing", has("healingAmount"), positive("healingAmount", formatInteger), 1);
    add("healingHits", "healingCount", "healing", has("healingHitCount"), positive("healingHitCount", formatInteger), 1, "healing");
    add("drainHealing", "drainHealing", "healing", has("drainHealingAmount"), positive("drainHealingAmount", formatInteger), 1);
    return columns;
  }

  function buildDetailSkillRow(skill, columns = buildDetailMetricColumns([skill])) {
    skill = globalThis.NotMeterCombatDetailBuffs.normalizeSkill(skill);
    const row = document.createElement("article");
    ...
    const skillDisplayName = globalThis.NotMeterCombatDetailBuffs.verifiedSkillName(skill, state.locale) || localizeGameName(
      recordedSkillName,
      "skill",
      skill.rawSkillCode,
      skill.skillCode);
    ...
    if (totalDamage > 0) {
      ...
      damage.textContent = `${t("healing")} ${formatInteger(healingAmount)}`;
    } else {
  ...
  function buildDetailHealingRow(skill, totalHealing) {
    ...
    const name = document.createElement("strong");
    name.className = "detail-skill-name";
    name.textContent = globalThis.NotMeterCombatDetailBuffs.verifiedSkillName(skill, state.locale) || localizeGameName(
    ...
    metrics.append(
      buildDetailChip(t("healing"), formatInteger(skill.healingAmount), "healing"),
      buildDetailChip(t("drainHealing"), Number(skill.drainHealingAmount) > 0 ? formatInteger(skill.drainHealingAmount) : "—", "healing"),
      buildDetailChip(t("useCount"), formatUseCount(skill.useCount)),
      buildDetailChip(t("detailHealingShare"), formatDetailPercent(Number(skill.healingAmount) / totalHealing * 100), "healing"));
  ...
  function buildSpecializationChip(flags, label = t("specialization")) {
    const chip = document.createElement("span");
  ...
  function normalizeSkillIconCode(value) {
    ...
    const verifiedIcon = globalThis.NotMeterCombatDetailBuffs?.skillIconSource({ rawSkillCode: code });
    if (verifiedIcon?.type === "skill") return Number(verifiedIcon.key);
  ...
  function updateCacheAge() {
    if (!state.data?.generatedAt || isPendingDungeon()) {
  ...
  function applyLocale() {
    ...
    window.NotMeterArtifactOccupation?.setLocale(state.locale);
    window.NotMeterRankerGuide?.refreshLocale();
    if (state.surfaceMode === "fieldBoss" && state.fieldBossData) {
      renderFieldBoss();
    }
  ...
  function updatePageIdentity() {
    if (state.surfaceMode === "praiseRanking") {
      document.title = "NotMeter 칭찬 랭킹";
      elements["page-title"].textContent = "NotMeter 칭찬 랭킹";
      elements["page-subtitle"].textContent = "전체 서버 TOP 20 · 누적 칭찬";
      return;
    }
  ...
  function selectableRankingDungeons() {
    ...
    const pending = SNOWFIELD_STATISTICS_KEYS.map(key => ({
        key,
      ...
      })).concat(FIRE_TEMPLE_DUNGEON);
    return dungeons.concat(pending.filter(item => !dungeons.some(dungeon => dungeon.key === item.key)));
  }

  function isPendingDungeon(dungeonKey = state.dungeonKey, data = state.data) {
    return PENDING_DUNGEON_KEYS.includes(dungeonKey) &&
      !data?.dungeons?.some(dungeon => dungeon.key === dungeonKey);
  }

  function renderPendingDungeon() {
    if (!isPendingDungeon()) return false;
    elements["class-heading"].hidden = true;
  ...
  function dungeonName(dungeon) {
    ...
    return dungeon.key === FIRE_TEMPLE_DUNGEON.key ? FIRE_TEMPLE_DUNGEON.displayName : dungeon.displayName;
  }
  ...
  function weeklyComparisonDisplay(paired) {
    ...
    return { ...change, title:
      `${t("pairedEstimate", { value: change.formattedValue })}\n` +
      (change.direction === "flat" ? `${t("pairedDirectionUnclear")}\n` : "") +
      `${t("weeklyTooltip")}\n` +
      `${population}${conditions}\n` +
      `${t("pairedRange", { low: formatWeeklyChangeNumber(paired.lowerChangePercent), high: formatWeeklyChangeNumber(paired.upperChangePercent) })}\n` +
  ...
  function pairedWeeklyChange(paired) {
    ...
    const magnitude = formatWeeklyChangeNumber(Math.abs(value));
    const formattedValue = `${value === 0 ? "" : value > 0 ? "+" : "−"}${magnitude}%`;
    return { direction, value, formattedValue,
      label: direction === "flat" ? t("pairedObserved", { value: formattedValue })
        : `${direction === "up" ? "▲" : "▼"} ${magnitude}%` };
  }

  function formatWeeklyChangeNumber(value) {
    // Keep a nonzero estimate or interval edge from rounding to zero.
    return value !== 0 && Math.abs(value) < 0.05
      ? new Intl.NumberFormat(localeTag(), { maximumSignificantDigits: 1 }).format(value)
